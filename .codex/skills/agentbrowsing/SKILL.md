---
name: agentbrowsing
description: Agent browsing skill for executing test cases and verifying bugs on HRcloud using Playwright MCP. Use when the user asks to execute test steps, verify a bug, test a feature by browsing, or when in browsering mode. Loads the HRcloud system knowledge base from .codex/system/ before any browser action.
---
...
```
File: .codex/system/04_roles_permissions/role_matrix.md
Purpose: Confirm which user role is required for the test
```

### Step 6: Read Testing Rules
```
For test execution â†’ .codex/system/05_testing_guidelines/test_execution_rules.md
For bug verification â†’ .codex/system/05_testing_guidelines/bug_verification_rules.md
```

### Step 7 (Optional): Read Workflow if Cross-Module
```
.codex/system/03_workflows/employee_lifecycle.md    â†’ hire/onboard/offboard tests
.codex/system/03_workflows/time_and_attendance.md   â†’ time off / time clock tests
.codex/system/03_workflows/performance_cycle.md     â†’ perform review tests
```
...
1. **Parse the test case** â€” identify: preconditions, steps, expected result
...
5. **Screenshot at key checkpoints** â€” before and after each major action
...
1. **Read the bug report** â€” extract: module, steps to reproduce, expected, actual
2. **Load knowledge** (steps above)
3. **Login** with the role mentioned in the bug report
4. **Reproduce exactly** â€” follow the steps from the bug report precisely
5. **Document observations** â€” screenshots, error messages, URLs at each step
6. **Classify**: REPRODUCED / NOT REPRODUCED / PARTIALLY REPRODUCED
7. **Report** using the template in `bug_verification_rules.md`
8. **Note locators used** â€” record all selectors found for later automation use
...
1. **READ Guidance.md FIRST** â€” no exceptions
2. **NEVER ask the user** how to navigate â€” use `url_map.md`
3. **NEVER hardcode waits** â€” use `browser_wait_for` with element conditions
4. **ALWAYS snapshot before action** â€” know the state before clicking
5. **ALWAYS screenshot after action** â€” capture the result
...
8. **DO NOT skip steps** from the test case â€” follow them exactly
...
|------|--------|----------|--------|--------|
| 1 | [what was done] | [expected] | [actual] | âœ… / âŒ |
| 2 | ... | ... | ... | ... |

**Overall**: PASS âœ… / FAIL âŒ / BLOCKED âš ï¸
...
**Status**: REPRODUCED âœ… / NOT REPRODUCED âŒ / PARTIAL âš ï¸
