...
- **Document exactly what you see** â€” don't interpret, record facts
...
1. [Step 1 â€” what I did]
2. [Step 2 â€” what I did]
3. [Step 3 â€” what I did]

### Result
**Status**: REPRODUCED âœ… / NOT REPRODUCED âŒ / PARTIALLY REPRODUCED âš ï¸
...
### Suggested Severity
[Critical / High / Medium / Low] â€” [reason]
```
...
**How to verify**:
```
1. browser_navigate  â†’ [form page URL]
2. browser_click     â†’ field to fill
3. browser_type      â†’ "Test Value"
4. browser_click     â†’ Save / Submit button
5. browser_wait_for  â†’ success indicator
6. browser_navigate  â†’ another page
7. browser_navigate  â†’ back to original page
8. browser_snapshot  â†’ check if field still shows "Test Value"
   - If field is empty â†’ data did not persist â†’ bug confirmed
   - If field shows correct value â†’ data saved correctly
```

### 4. Permission / Access Bug
**How to verify**:
```
1. Login as Employee â†’ browser_snapshot â†’ note which elements are visible
2. Login as Manager  â†’ browser_snapshot â†’ note which elements are visible
3. Login as Admin    â†’ browser_snapshot â†’ note which elements are visible
...
- Check HRcloud's integration logs: Settings â†’ Integrations â†’ [Provider] â†’ Logs
...
## NOT A BUG â€” Common Misidentifications
...
|----------|------------|
| Manager can't see all employees | Expected â€” managers see only direct reports |
| Time Off balance not deducted immediately after request | Expected â€” balance deducts after approval |
| Prehire can't access all modules | Expected â€” prehire portal is restricted by design |
...
| Changes not taking effect (People Full) | May be a future-dated change â€” check the effective date |
| Email not arriving immediately | Expected delay of up to 5 minutes |
| I-9 Section 2 not visible | Expected â€” can only fill after employee completes Section 1 |
